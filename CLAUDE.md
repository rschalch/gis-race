...
make bake ARGS='--from "Origin, ST" --to "Destination, ST" --slug my-route-slug'
                   ...
                   # --from/--to take a place name OR exact "lat, lon" coordinates, e.g.
                   #   --from "-23.48886, -47.43913" --to "Monte Verde, MG"
                   # Coordinates skip geocoding and pin the endpoint to within a metre;
                   # add --name "Course name" when reverse-geocoded labels aren't useful.
                   # --via <stop> may be repeated; the stops are driven through in the order
                   # given, and take place names or coordinates just like the endpoints. This is
                   # how you pin a race to the roads you actually want. Alternatives are skipped
                   # when there are stops: pinning the road leaves only one shape.
                   # --round-trip bakes start -> finish -> start as ONE route. The return leg
                   # is routed separately (B->A, not A->B reversed), so one-way systems and
                   # turn restrictions are respected and the way back is often a different road.
                   # How long cars WAIT at the turnaround is a race setting, not a bake one
                   # (Configure Race > Turnaround stop, or sim-batch --turnaroundPause <min>):
                   # a bake is minutes of rate-limited fetching, and the stop is worth trying
                   # several values of.
...
make sim-batch ARGS='--route sorocaba-campos --seeds 30'
                   ...
                   # --wind calm|breezy|windy (R16) sets the race's wind strength; its compass direction
                   # comes from each seed, so reseeding rerolls the direction.
...
- **`src/coords.ts`** (shared with the browser, not Node-only) — parses a typed endpoint into either coordinates or a place query. Accepts `lat, lon` only, never `lon, lat`: supporting both means guessing, and the guess is wrong precisely in the ±90° band where both readings parse, i.e. most of the inhabited world. An out-of-range latitude that would be valid swapped says so rather than falling through to a "couldn't find that place" geocoding error.
- **`bakeRoute.ts`** — shared baking logic: fetches geometry (Valhalla primary, OSRM demo-server fallback) + elevation (OpenTopoData SRTM 30m), derives grade/curvature/surface/speed-limit per 25 m point, used by both the CLI and the dev-server API. Server-side only because Nominatim/OpenTopoData don't allow direct browser calls (User-Agent / CORS). `buildRoundTrips` joins each outbound variant to a separately-routed return leg, pairing variant *i* out with variant *i* back so a course's F1 variants stay distinct loops. Every about-face on a course needs an imposed cornering radius (`TURNAROUND_RADIUS_M`) and this is not optional: Menger curvature samples three points 50 m apart, so where a course doubles back along the same road the outer two coincide, the triangle has no area, and the tightest manoeuvre on the course records as a 5 km-radius straight — cars would take the U-turn at the speed limit. `detectReversals` finds them from the geometry (a >143° direction change over ±50 m) rather than only at a round trip's known join, because an intermediate stop can force one too — a dead-end village, a spur, a stop on the far carriageway — and did on the first multi-stop route baked. The radius is stamped onto the *raw* radius before the forward-looking min filter, so the approach inherits it. Intermediate stops (`via`) are routed as extra `locations` in one request rather than as separately-fetched legs stitched together, so the router owns the joins; `alternatives` is suppressed whenever there is more than one leg, since a route pinned to intermediate points has exactly one shape. A round trip's return leg deliberately carries **no** vias — the stops describe how to get *there*, and repeating them in reverse would guarantee the way back mirrors the way out, which is the one thing a separately-routed return exists to avoid. `resolveEndpoint` turns each endpoint into a coordinate plus a label, from (in precedence order) a coordinate the caller already holds (an autocomplete pick), coordinates in the text, or a geocoded place name; a coordinate endpoint is reverse-geocoded **for its label only**, never for its position, and a failed lookup falls back to the formatted pair rather than aborting the bake.
...
- A there-and-back course records `route.turnaroundS` (optional, absent on one-way routes) and the sim holds every car there for `Sim.turnaroundPauseS`, using its own `paused` status — *not* `spinning` with a long recovery, because a spin is an incident that logs, registers a caution hazard and damages the car, and a scheduled stop is none of those things. The stop costs slightly more than its own duration, correctly: a car that stops has to accelerate from rest where one driving through carries its cornering speed out of the turn.
